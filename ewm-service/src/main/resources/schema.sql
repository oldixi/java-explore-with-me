...
create or replace function getEventsByPlaceId(placeId integer)
...
begin
   return query
		...
		from places p,
     		 locations l join
     		 events e on e.location_id = l.id
		where distance(l.lat, l.lon, p.lat, p.lon) < p.radius
  		  and p.id = placeId;
end '
language plpgsql;

create or replace function getEventsByPlaceName(placeName varchar)
    returns table(id INTEGER,
        		  annotation VARCHAR(2000),
        	      category_id INTEGER,
				  confirmed_requests INTEGER,
